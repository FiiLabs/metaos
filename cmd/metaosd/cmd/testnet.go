...
import (
	...
	evmutils "github.com/FiiLabs/metaos/modules/evm/utils"
	...
	opbtypes "github.com/FiiLabs/metaos/modules/opb/types"
)
...
func testnetCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a metaos testnet",
		Long: "testnet will create \"v\" number of directories and populate each with " +
			"necessary files (private validator, genesis, config, etc.).\n" +
			"Note, strict routability for addresses is turned off in the config file.",
		Example: "metaosd testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2",
	...
	cmd.Flags().String(flagNodeDaemonHome, "metaosd", "Home directory of the node's daemon configuration")
	cmd.Flags().String(flagNodeCLIHome, "metaoscli", "Home directory of the node's cli configuration")
...
func InitTestnet(
	...
	metaosConfig := evmosConfig.DefaultConfig()
	metaosConfig.MinGasPrices = minGasPrices
	metaosConfig.API.Enable = true
	metaosConfig.Telemetry.Enabled = true
	metaosConfig.Telemetry.PrometheusRetentionTime = 60
	metaosConfig.Telemetry.EnableHostnameLabel = false
	metaosConfig.Telemetry.GlobalLabels = [][]string{{"chain_id", chainID}}
	...
	for i := 0; i < numValidators; i++ {
		...
		metaosConfigFilePath := filepath.Join(nodeDir, "config/app.toml")
		srvconfig.WriteConfigFile(metaosConfigFilePath, metaosConfig)
	}
